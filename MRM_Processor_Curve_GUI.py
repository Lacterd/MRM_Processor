...
class MRMProcessorUI(QWidget):
    ...
    def initUI(self):
        ...
        self.LineEdit_Int_min = QLineEdit('5000')
        ...
        self.LineEdit_RE_Tor = QLineEdit('15')
        self.Combox_Weight = QComboBox()
        self.Combox_Weight.addItems(['1','1/x','1/x²'])
        self.Combox_Weight.setCurrentText('1/x²')
    ...
    def on_multiprocess_done(self, result_dict):
        ...
        results = {}
        results_DF = pd.DataFrame(columns=['Identity keys','slope','intercept','r2','RE','X_L','X_R'])
        for i in self.All_Data_Area_IS_1.index:
            ...
            for i_R in Concentration_set[1:]:
                ...
                for ii in self.Data_params.keys():
                    if self.Data_params[ii]['Type'] != 'Blank':
            ...
            if len(score) > 0:
                ...
                temp_DF = pd.DataFrame({"Identity keys":main_MRM.All_Data_Area_IS_1.loc[i,'Identity keys'],"slope": '', "intercept": '', "r2": '', 'RE':[''], 'X_L':0,'X_R':0})
                results_DF = pd.concat([results_DF,temp_DF])
        ...
        results_DF.to_excel(self.filepath_title+'Curve Data-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.xlsx',index=False)
    ...
    def Run(self):
        ...
        self.params = {
            ...
            'min_Int_Quantitative':self.LineEdit_Int_min.text(),
            'min_Int_Qualitative':self.LineEdit_Int_min.text(),
            'Smooth':5
        }
