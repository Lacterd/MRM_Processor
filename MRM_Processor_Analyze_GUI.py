...
class MRMProcessorUI(QWidget):
    def __init__(self):
        ...
        # 设置参数字典
        self.Data_params={}
        self.Calculate_params={'MS1_Tor':0.000010,'smooth':5,'min_Int':5000,'Points':10}
    ...
    def initUI(self):
        ...
        self.Label_Curve_Select = QLabel('Select curve file')
        ...
        self.TextBrowser_SampleSelect.setHorizontalHeaderLabels(['Name','Type'])
        # 设置表头行为
        ...
        self.PushButton_Curve_Select = QPushButton('Select')
        self.TextBrowser_Target_Select = QLabel('')
        self.TextBrowser_Curve_Select = QLabel('')
        ...
        self.PushButton_Curve_Select.clicked.connect(self.SelectCurveFile)
        ...
        Data_import_Layout.addWidget(self.Label_Curve_Select,7,0)
        Data_import_Layout.addWidget(self.TextBrowser_Curve_Select, 8, 0)
        Data_import_Layout.addWidget(self.PushButton_Curve_Select, 8, 1)
        Data_import_Layout.addWidget(self.Label_Manual_Select,9,0)
        Data_import_Layout.addWidget(self.TextBrowser_Manual_Select,10,0)
        Data_import_Layout.addWidget(self.PushButton_Manual_Select,10,1)
        Data_import_Layout.addWidget(self.PushButton_Manual_Reset,10,2)
        ...
        self.Lable_Int_min_Quantitative = QLabel('Quantitative intensity')
        self.Lable_Int_min_Qualitative = QLabel('Qualitative intensity')
        self.Lable_Point = QLabel('Min scan points')
        self.Lable_SingleData = QLabel('Report Type')
        self.Lable_Smooth = QLabel('Smooth level')
        self.LineEdit_MS_Tor = QLineEdit('0.35')
        self.LineEdit_IS_RT_Tor = QLineEdit('0.1')
        self.LineEdit_RT_Tor = QLineEdit('0.1')
        self.LineEdit_Int_min_Quantitative = QLineEdit('5000')
        self.LineEdit_Int_min_Qualitative = QLineEdit('5000')
        ...
        self.Combox_Smooth = QComboBox()
        self.Combox_Smooth.addItems(['3','2','1','0'])
        self.Combox_Smooth.setCurrentText('2')
        ...
        Params_setting_Layout.addWidget(self.Lable_Int_min_Quantitative,2,0)
        Params_setting_Layout.addWidget(self.LineEdit_Int_min_Quantitative,2,1)
        Params_setting_Layout.addWidget(self.Lable_Int_min_Qualitative,2,2)
        Params_setting_Layout.addWidget(self.LineEdit_Int_min_Qualitative,2,3)
        Params_setting_Layout.addWidget(self.Lable_Point,2,4)
        Params_setting_Layout.addWidget(self.LineEdit_Point,2,5)
        Params_setting_Layout.addWidget(self.Lable_Smooth,3,0)
        Params_setting_Layout.addWidget(self.Combox_Smooth,3,1)
        Params_setting_Layout.addWidget(self.Lable_SingleData,3,2)
        Params_setting_Layout.addWidget(self.Combox_Export,3,3)
    ...
    def SelectSampleFile(self):
        ...
        if len(FileName) > 0:
            for i in FileName:
                ...
                self.Data_params[i[name_begin+1:name_end-5]] = {'Name':i[name_begin+1:name_end-5],'Path':i,'LineEdit_Manual':LineEdit_Manual,'Combox':Combox_Type}
                ...
                self.TextBrowser_SampleSelect.setCellWidget(self.TextBrowser_SampleSelect.rowCount()-1,2,self.Data_params[i[name_begin+1:name_end-5]]['LineEdit_Manual'])
    ...
    def SampleTypeChange(self):
        for i in self.Data_params.keys():
            if self.sender() == self.Data_params[i]['Combox']:
                ...
                if self.Data_params[i]['Combox'].currentText() == 'Sample':
                    self.Data_params[i]['Type'] = 'Sample'
                elif self.Data_params[i]['Combox'].currentText() == 'Blank':
                    self.Data_params[i]['Type'] = 'Blank'
                elif self.Data_params[i]['Combox'].currentText() == 'QC':
                    self.Data_params[i]['Type'] = 'QC'
            QApplication.processEvents()
    ...
    def SelectCurveFile(self):
        FileName,FileType = QFileDialog.getOpenFileName(self,"选取文件",os.getcwd(),"Excel Files(*.xlsx)")
        name_begin = FileName.rfind('/')
        if name_begin == -1:
            name_begin = FileName.rfind('\\')
        name_end = len(FileName)
        self.TextBrowser_Curve_Select.setText(FileName[name_begin+1:name_end])
        self.CurvePath = FileName
    ...
    def on_multiprocess_done(self, result_dict):
        ...
        self.All_Data_Area_2 = self.All_Data_Area_1[self.All_Data_Area_1['IS'].notna()]
        self.All_Data_Area_1 = self.All_Data_Area_1.loc[:, ['Identity keys']]
        self.All_Data_Area_2 = self.All_Data_Area_2.loc[:, ['Identity keys']]
        self.All_Data_Area_1.reset_index(drop=True,inplace=True)
        self.All_Data_Area_2.reset_index(drop=True,inplace=True)
        self.RT_fix = self.All_Data_Area_1.copy()
        self.All_Data_Int_1 = self.All_Data_Area_1.copy()
        self.All_Data_Int_2 = self.All_Data_Area_2.copy()
        self.All_Data_Area_IS_1 = self.All_Data_Area_2.copy()
        self.All_Data_Area_IS_2 = self.All_Data_Area_2.copy()
        self.All_Data_Time = pd.read_excel(self.TargetPath)
        self.All_Data_Time = self.All_Data_Time.loc[:, ['Identity keys']]
        ...
        for i in self.Data_params.keys():
            ...
            self.RT_fix[i] = self.Data_params[i]['TargetList'].loc[:, 'RT_1']
            ...
            self.All_Data_Int_1[i] = self.Data_params[i]['TargetList'].loc[:, 'Hight_1']
            self.All_Data_Int_2[i] = self.Data_params[i]['TargetList'].loc[:, 'Hight_2']
            ...
            self.All_Data_Time[i] = list(self.Data_params[i]['TargetList'].loc[:, 'RT_1'])+list(self.Data_params[i]['Calibrant'].loc[:, 'RT_1'])
            ...
            for ii in range(len(self.Data_params[i]['TargetList'])):
                if type(self.Data_params[i]['TargetList'].loc[ii,'Edge_left_1']) != str and self.Data_params[i]['TargetList'].loc[ii,'Edge_left_1'] > 0:
                    ...
                    self.All_Data_Manual.loc[Name_Match,i+'-L'] = np.around(self.Data_params[i]['TargetList'].loc[Name_Match,'Plot_RT_1'][0]/60,4)
                    self.All_Data_Manual.loc[Name_Match,i+'-R'] = np.around(self.Data_params[i]['TargetList'].loc[Name_Match,'Plot_RT_1'][-1]/60,4)
            for ii in range(len(self.Data_params[i]['Calibrant'])):
                if type(self.Data_params[i]['Calibrant'].loc[ii,'Edge_left_1']) != str and self.Data_params[i]['Calibrant'].loc[ii,'Edge_left_1'] > 0:
                    ...
                    self.All_Data_Area_1.loc[Name_Match,i] = self.Data_params[i]['Calibrant'].loc[ii,'Area_1']
                    self.All_Data_Int_1.loc[Name_Match,i] = self.Data_params[i]['Calibrant'].loc[ii,'Hight_1']
        localtime = time.localtime()
        suffix = f"{localtime.tm_mon}{localtime.tm_mday}{localtime.tm_hour}{localtime.tm_min}"
        basepath = self.filepath_title
        self.RT_fix.to_excel(basepath + f'MRM_RT_Fix_{suffix}.xlsx', index=False)
        ...
        self.All_Data_Int_1.to_excel(basepath + f'MRM_Int_1_{suffix}.xlsx', index=False)
        self.All_Data_Int_2.to_excel(basepath + f'MRM_Int_2_{suffix}.xlsx', index=False)
        ...
        if len(self.CurvePath)>0:
            self.Curve = pd.read_excel(self.CurvePath)
            self.All_Data_Concentration = self.All_Data_Area_IS_1.copy()
            Sample_key = []
            for i in self.Data_params.keys():
                if self.Data_params[i]['Type'] == 'Sample' or self.Data_params[i]['Type'] == 'QC':
                    Sample_key.append(i)
                    self.All_Data_Concentration.loc[:,i] = ['']*len(self.All_Data_Concentration)
            for i in self.All_Data_Concentration.index:
                if self.All_Data_Concentration.loc[i,'Identity keys'] in list(self.Curve['Identity keys']):
                    for ii in Sample_key:
                        if type(self.All_Data_Area_IS_1.loc[i,ii]) in [float,int,np.float64,np.float32,np.float16]:
                            Curve_index = list(filter(lambda x: self.All_Data_Concentration.loc[i,'Identity keys'] == self.Curve.loc[x,'Identity keys'],self.Curve.index))[0]
                            self.All_Data_Concentration.loc[i,ii] = (self.All_Data_Area_IS_1.loc[i,ii]-self.Curve.loc[Curve_index,'intercept'])/self.Curve.loc[Curve_index,'slope']
                fig = go.Figure()
                for ii in self.Data_params.keys():
                    if (self.Data_params[ii]['Type'] == 'Sample' or self.Data_params[ii]['Type'] == 'QC') and type(self.Data_params[ii]['TargetList'].loc[i,'Plot_RT_1']) != str:
                        fig.add_trace(
                            go.Scatter(
                    ...
                    if (self.Data_params[ii]['Type'] == 'Sample' or self.Data_params[ii]['Type'] == 'QC') and type(self.Data_params[ii]['TargetList'].loc[i,'Plot_RT_All_1']) != str:
                        fig.add_trace(
                            go.Scatter(
                ...
                fig.update_layout(
                    ...
                    xaxis={'title':{'text':'Retention time','font':{'size':15},'standoff':0},
                           'linecolor':'black',
                           'tickfont':{'size':11},
                           'ticks':'outside',
                           'ticklen':2,
                    ...
                    yaxis={'title':{'text':'Intensity','font':{'size':15},'standoff':0},
                           'linecolor':'black',
                           'tickfont':{'size':11},
                           'ticks':'outside',
                           'ticklen':2,
            ...
            self.All_Data_Concentration.to_excel(basepath + f'MRM_Concentration_{suffix}.xlsx', index=False)
        for i in range(len(self.Data_params[list(self.Data_params.keys())[0]]['Calibrant'])):
            fig = go.Figure()
            for ii in self.Data_params.keys():
                if (self.Data_params[ii]['Type'] == 'Sample' or self.Data_params[ii]['Type'] == 'QC') and type(self.Data_params[ii]['Calibrant'].loc[i,'Plot_RT_1']) != str:
                    fig.add_trace(
                        go.Scatter(
                            x = [x/60 for x in self.Data_params[ii]['Calibrant'].loc[i,'Plot_RT_1']],
                            y = self.Data_params[ii]['Calibrant'].loc[i,'Plot_Int_1'],
                            hovertemplate=(
                            f"Name: {ii}<br>"
                            "RT: %{x}<br>"
                            "Int: %{y}<br>"
                ...
                if (self.Data_params[ii]['Type'] == 'Sample' or self.Data_params[ii]['Type'] == 'QC') and type(self.Data_params[ii]['Calibrant'].loc[i,'Plot_RT_All_1']) != str:
                    fig.add_trace(
                        go.Scatter(
                            x = [x/60 for x in self.Data_params[ii]['Calibrant'].loc[i,'Plot_RT_All_1']],
                            y = self.Data_params[ii]['Calibrant'].loc[i,'Plot_Int_All_1'],
                            hovertemplate=(
                            f"Name: {ii}<br>"
                            "RT: %{x}<br>"
                            "Int: %{y}<br>"
            ...
            fig.update_layout(
                ...
                width=700,
                height=600,
                title='Curve of '+self.Data_params[list(self.Data_params.keys())[0]]['Calibrant'].loc[i,'Identity keys'],
                titlefont={'size':20},
                plot_bgcolor='rgba(0,0,0,0)',
                xaxis={'title':{'text':'Retention time','font':{'size':15},'standoff':0},
                       'linecolor':'black',
            ...
            fig.layout.font.family = 'Helvetica'
            fig.write_html(self.filepath_title+'Area of '+self.Data_params[list(self.Data_params.keys())[0]]['Calibrant'].loc[i,'Identity keys']+'-'+str(time.localtime()[1])+str(time.localtime()[2])+str(time.localtime()[3])+str(time.localtime()[4])+'.html',config={'responsive': False})
        self.Label_process_sub.setText("Finish")
        self.process_bar.setValue(len(self.Data_params.keys()))
        QApplication.processEvents()
    ...
    def Run(self):
        ...
        self.params = {
            ...
            'min_Int_Quantitative': self.LineEdit_Int_min_Quantitative.text(),
            'min_Int_Qualitative': self.LineEdit_Int_min_Qualitative.text(),
            'Point': self.LineEdit_Point.text(),
            'TargetPath': self.TargetPath,
            'Export': self.Combox_Export.currentText(),
            'ManualList':self.ManualListPath,
            'Smooth':float(self.Combox_Smooth.currentText())
        }
...
class MRMProcess(object):   
    ...
    def load_TargetList(self,TargetPath,DefFilePath=''):
        ...
        if 'IS' in self.TargetList.keys():
            ...
            self.Calibrant['Plot_RT_1'] = ''
            self.Calibrant['Plot_Int_1'] = ''
            self.Calibrant['Plot_RT_All_1'] = ''
            self.Calibrant['Plot_Int_All_1'] = ''
            ...
            for i in range(len(self.Calibrant_set)):  
                ...
                if len(MRM_1)>0 and len(MRM_2)>0:
                    ...
                    for i_1,i_2 in Sm_Assign:
                        ...
                        self.Calibrant.at[self.Calibrant_set.at[i,'TL_Index'][i_2],'Plot_RT_1'] = Plot_RT_1[Sm_Filter[i_1][0]]
                        self.Calibrant.at[self.Calibrant_set.at[i,'TL_Index'][i_2],'Plot_Int_1'] = Plot_Int_1[Sm_Filter[i_1][0]]
                        self.Calibrant.at[self.Calibrant_set.at[i,'TL_Index'][i_2],'Plot_RT_All_1'] = Origin_RT_List_1
                        self.Calibrant.at[self.Calibrant_set.at[i,'TL_Index'][i_2],'Plot_Int_All_1'] = Origin_Int_List_1
                        if pd.notna(self.Calibrant_set.loc[i,'2_Q1']):
    ...
    def load_ManualList(self,ManualPath,Name):
        ...
        for i in range(len(self.TargetList)):
            if self.TargetList.loc[i,Name+'-L'] >0:
                ...
                self.TargetList.at[i,'Plot_RT_2'] = ''
                self.TargetList.at[i,'Plot_Int_2'] = ''
                self.TargetList.at[i,'Plot_RT_2'] = RT_List
                self.TargetList.at[i,'Plot_Int_2'] = Int_List
    ...
    def MRMPeakDetecter(self,Auto_RT_List,Auto_Int_List,min_Int=-1,BaseLine=0,SmoothLevel=1):
        # 单个色谱峰识别模块，由下方另一函数调用
        def GaussSmooth(x):
            if len(x)==7:
                op = x[0]*0.06+x[1]*0.14+x[2]*0.19+x[3]*0.22+x[4]*0.19+x[5]*0.14+x[6]*0.06
            if len(x)==5:
        ...
        if SmoothLevel == 1:
            ...
            Auto_Int_List = np.array(list(map(lambda x:GaussSmooth(Auto_Int_List[x-3:x+3+1]) if x in range(3,len(Auto_Int_List)-3) else Auto_Int_List[x],range(len(Auto_Int_List)))))
        elif SmoothLevel == 0:
            pass
            ...
            for i_edge in range(len(Peak_Begin)):
                ...
                if Peak_Begin[i_edge]>=self.get_param('FeatureDetectPlot'):
                    ...
                    try:
                        ...
                        if i_edge-1 in range(len(Peak_Begin)) and len(raw_Int_List[max(Peak_End[i_edge-1],Peak_Begin[i_edge]-self.get_param('FeatureDetectPlot')):min(len(raw_Int_List),Peak_Begin[i_edge]+2*self.get_param('FeatureDetectPlot')+1)])>0:
                            ...
                            if len(raw_Int_List[Peak_Begin[i_edge]-self.get_param('FeatureDetectPlot'):min(len(raw_Int_List),Peak_Begin[i_edge]+2*self.get_param('FeatureDetectPlot')+1)])>0:
                ...
                if Peak_End[i_edge]<len(raw_Int_List)-self.get_param('FeatureDetectPlot'):
                    ...
                    try:
                        ...
                        if i_edge+1 in range(len(Peak_Begin)) and len(raw_Int_List[max(0,Peak_End[i_edge]-2*self.get_param('FeatureDetectPlot')):min(Peak_End[i_edge]+self.get_param('FeatureDetectPlot')+1,Peak_Begin[i_edge+1]+1)])>0:
                            ...
                            if len(raw_Int_List[max(0,Peak_End[i_edge]-2*self.get_param('FeatureDetectPlot')):Peak_End[i_edge]+self.get_param('FeatureDetectPlot')+1])>0:
    ...
    def Calculate_Noise(self,RT_List,Int_List):
        ...
        FD_List = np.array(list(map(lambda x: MRMProcess.noise_TFFD(x, Int_List, RT_List), range(len(Int_List)))))
        ...
        Index_Filter = [x for x in range(2,len(FD_List)-2) if ((min(FD_List[x-1],FD_List[x-2])*FD_List[x]<=0 and FD_List[x]*min(FD_List[x+1],FD_List[x+2])<=0 and min(FD_List[x-1],FD_List[x-2])*min(FD_List[x+1],FD_List[x+2])>=0) or (max(FD_List[x-1],FD_List[x-2])*FD_List[x]<=0 and FD_List[x]*max(FD_List[x+1],FD_List[x+2])<=0 and max(FD_List[x-1],FD_List[x-2])*max(FD_List[x+1],FD_List[x+2])>=0)) and Int_List[x] >0]
        if len(Index_Filter) == 0:
            # 极端情况输出残差
            noise = np.median(abs(Int_List[Int_List>0]-Auto_Int_List[Int_List>0]))
        else:
            # 输出高频震荡数据
            Int_List = Int_List[Index_Filter]
            Int_List.sort()
            # 过滤极端异常值，避免mean函数过大
            Int_List = Int_List[int(len(Int_List)*0.05):int(len(Int_List)*0.95)]
            noise = 0.5*(np.median(Int_List)+np.mean(Int_List))
            #mad = np.median(np.abs(Int_List-np.median(Int_List)))
            #noise = 1.4826 * mad
        return noise
    ...
    def detect_Peak_MRM(self,FilePath=''):
        ...
        def calculate_noise_by_derivative_mad(
            RT_List,
            Int_List,
            Peak_Begin,
            Peak_End,
            ...
            """
            基于已识别峰区间的导数-MAD噪声估计。
            Peak_Begin 和 Peak_End 必须是相对于整个 Origin_RT_List / Origin_Int_List 的全局 index。
            """
            y = np.asarray(Int_List, dtype=float)
            n = y.size
            # 1. mask 掉所有已识别峰区
            peak_mask = np.zeros(n, dtype=bool)
            for b, e in zip(Peak_Begin, Peak_End):
                b = int(b)
                e = int(e)
                if b > e:
                    b, e = e, b
                b = max(0, b - pad_points)
                e = min(n - 1, e + pad_points)
                peak_mask[b:e + 1] = True
            ...
            diffs = []
            baseline_values = []
            for s, e in contiguous_regions(baseline_mask):
                seg = y[s:e + 1]
                if drop_zeros:
                    seg = seg[seg > 0]
                if seg.size >= 3:
                    baseline_values.extend(seg)
                    diffs.extend(np.diff(seg))
            diffs = np.asarray(diffs, dtype=float)
            baseline_values = np.asarray(baseline_values, dtype=float)
            # 3. 如果mask后非峰点太少，则退回到全trace非零点
            if diffs.size < min_points:
                seg = y[y > 0] if drop_zeros else y
                if seg.size >= 3:
                    diffs = np.diff(seg)
                    baseline_values = seg
                else:
            ...
            # 5. baseline 用非峰区中位数
            baseline = np.median(baseline_values) if baseline_values.size > 0 else 0
            return float(noise), float(baseline)
        # 色谱峰识别
                    ...
                    for i_1 in range(len(Sm_Filter)):
                        for i_2 in range(len(self.TargetList_set.loc[i,'RT'])):
                            if abs(RT_List_1[Sm_Filter[i_1][0]]-self.TargetList_set.loc[i,'RT'][i_2]*60)< self.get_param('Target_RT_Tor') and abs(RT_List_2[Sm_Filter[i_1][1]]-self.TargetList_set.loc[i,'RT'][i_2]*60)< self.get_param('Target_RT_Tor') and Peak_End_1[Sm_Filter[i_1][0]]-Peak_Begin_1[Sm_Filter[i_1][0]] >= self.get_param('Points')-1:
                                max_1 = max(max_1,Int_List_1[Sm_Filter[i_1][0]])
                                max_2 = max(max_2,Int_List_2[Sm_Filter[i_1][1]])
                    for i_1 in range(len(Sm_Filter)):
                        for i_2 in range(len(self.TargetList_set.loc[i,'RT'])):
                            if abs(RT_List_1[Sm_Filter[i_1][0]]-self.TargetList_set.loc[i,'RT'][i_2]*60)< self.get_param('Target_RT_Tor') and abs(RT_List_2[Sm_Filter[i_1][1]]-self.TargetList_set.loc[i,'RT'][i_2]*60)< self.get_param('Target_RT_Tor') and Peak_End_1[Sm_Filter[i_1][0]]-Peak_Begin_1[Sm_Filter[i_1][0]] >= self.get_param('Points')-1:
                                Score_matrix[i_1,i_2] = 0.35*(2-abs(RT_List_1[Sm_Filter[i_1][0]]-self.TargetList_set.loc[i,'RT'][i_2]*60)/ self.get_param('Target_RT_Tor') - abs(RT_List_2[Sm_Filter[i_1][1]]-self.TargetList_set.loc[i,'RT'][i_2]*60)/ self.get_param('Target_RT_Tor')) + 0.25*(Int_List_1[Sm_Filter[i_1][0]]/max_1 + Int_List_2[Sm_Filter[i_1][1]]/max_2)
                    Sm_1,Sm_2 = linear_sum_assignment(Score_matrix,True)         
                    Sm_Assign = [(x,y) for x,y in zip(Sm_1,Sm_2) if Score_matrix[x,y]>0]
                    for i_1,i_2 in Sm_Assign:
    ...
    def noise_TFFD(x, Auto_Int_List, Auto_RT_List):
        if 1<= x <= len(Auto_Int_List)-2:
            FirstDerivative = (Auto_Int_List[x+1]-Auto_Int_List[x-1])/((Auto_RT_List[x+1]-Auto_RT_List[x-1])*2)
        elif x < 1:
            FirstDerivative = (Auto_Int_List[x+1]-Auto_Int_List[x])/(Auto_RT_List[x+1]-Auto_RT_List[x])
        elif x > len(Auto_Int_List)-2:
            FirstDerivative = (Auto_Int_List[x]-Auto_Int_List[x-1])/(Auto_RT_List[x]-Auto_RT_List[x-1])
    ...
    def TFSD(x, Auto_Int_List, Auto_RT_List):
        if 2<= x <= len(Auto_Int_List)-3:
            SecondDerivative = (Auto_Int_List[x+1]*16+Auto_Int_List[x-1]*16-Auto_Int_List[x] * 30-Auto_Int_List[x+2]-Auto_Int_List[x-2])/((Auto_RT_List[x+2]-Auto_RT_List[x-2])*3)**2
        elif x < 2:
            SecondDerivative = (Auto_Int_List[x]-Auto_Int_List[x+1]+Auto_Int_List[x+2])/((Auto_RT_List[x+2]-Auto_RT_List[x])*3)**2
        elif x > len(Auto_Int_List)-3:
            SecondDerivative = (Auto_Int_List[x]-Auto_Int_List[x-1]+Auto_Int_List[x-2])/((Auto_RT_List[x]-Auto_RT_List[x-2])*3)**2
