...
class MRMProcess(object):   
    def __init__(self,DataPath):
        ...
        self.__param = {'MS1_Tor':0.35,'RT_Tor':6,'List_RT_Tor':60,'min_Int':20000,'min_MZ':100,'min_RT':60,'min_RT_width':6,'max_Noise':2000,
    ...
    def load_TargetList(self,Path):
        self.TargetList = pd.read_excel(Path, engine='openpyxl')
        ...
        if len(Begin_Place) >= 1 and len(End_Place) >= 1 and Begin_Place[0] < End_Place[-1]:
            ...
            for i_FD_Seq in range(len(Begin_Place)):
                ...
                for ii_FD_Seq in range(len(End_Place)):
                    if End_Place[ii_FD_Seq]>=Begin_Place[i_FD_Seq]+self.get_param('FeatureDetectPlot')*2:
            ...
            for i_edge in range(len(Peak_Begin)):
                if Peak_Begin[i_edge]>=self.get_param('FeatureDetectPlot'):
                    Begin_Filter = np.where(Auto_Int_List[Peak_Begin[i_edge]-self.get_param('FeatureDetectPlot'):Peak_Begin[i_edge]+self.get_param('FeatureDetectPlot')+1]==min(Auto_Int_List[Peak_Begin[i_edge]-self.get_param('FeatureDetectPlot'):Peak_Begin[i_edge]+self.get_param('FeatureDetectPlot')+1]))[0][-1]
                    if Auto_Int_List[Peak_Begin[i_edge]-self.get_param('FeatureDetectPlot')+Begin_Filter] < Auto_Int_List[Peak_Begin[i_edge]] and Auto_Int_List[Peak_Begin[i_edge]-self.get_param('FeatureDetectPlot')+Begin_Filter] < Peak_End[i_edge]:
                ...
                if Peak_End[i_edge]<len(Auto_Int_List)-self.get_param('FeatureDetectPlot'):
                    End_Filter = np.where(Auto_Int_List[Peak_End[i_edge]-self.get_param('FeatureDetectPlot'):Peak_End[i_edge]+self.get_param('FeatureDetectPlot')+1]==min(Auto_Int_List[Peak_End[i_edge]-self.get_param('FeatureDetectPlot'):Peak_End[i_edge]+self.get_param('FeatureDetectPlot')+1]))[0][-1]
                    if Auto_Int_List[Peak_End[i_edge]+self.get_param('FeatureDetectPlot')-End_Filter] > Auto_Int_List[Peak_End[i_edge]] and Peak_End[i_edge]+End_Filter-self.get_param('FeatureDetectPlot')>Peak_Begin[i_edge]:
                        Peak_End[i_edge] = Peak_End[i_edge]+End_Filter-self.get_param('FeatureDetectPlot')
            Area_List = []
            Int_List = []
            Final = []
            for i_edge in range(len(Peak_Begin)):
                ...
                Int_List.append(max(np.array(Auto_Int_List)[Peak_Begin[i_edge]:Peak_End[i_edge]]))
                if max(np.array(Auto_Int_List)[Peak_Begin[i_edge]:Peak_End[i_edge]]) > self.get_param('min_Int'):
                    Final.append(i_edge)
        ...
        if 'Edge_left_1' not in self.TargetList.keys():
            ...
            for i in range(len(self.TargetList_set)):  
                ...
                if len(MRM_1)>0 and len(MRM_2)>0:
                    ...
                    Peak_Result_2 = self.MRMPeakDetecter(MRM_2.iloc[0,:]['RTList'],MRM_2.iloc[0,:]['Int'])
                    ...
                    RT_List_2 = [MRM_2.iloc[0,:]['RTList'][x] for x in Peak_Top_2]
                    RT_L_1 = [MRM_1.iloc[0,:]['RTList'][x] for x in Peak_Begin_1]
                    RT_R_1 = [MRM_1.iloc[0,:]['RTList'][x] for x in Peak_End_1]
                    RT_L_2 = [MRM_2.iloc[0,:]['RTList'][x] for x in Peak_Begin_2]
                    RT_R_2 = [MRM_2.iloc[0,:]['RTList'][x] for x in Peak_End_2]
                    ...
                    for i_1 in range(len(Sm_Filter)):
                        for i_2 in range(len(self.TargetList_set.loc[i,'RT'])):
                            if abs(RT_List_1[Sm_Filter[i_1][0]]-self.TargetList_set.loc[i,'RT'][i_2]*60)< self.get_param('List_RT_Tor') and abs(RT_List_2[Sm_Filter[i_1][1]]-self.TargetList_set.loc[i,'RT'][i_2]*60)< self.get_param('List_RT_Tor'):
                                Score_matrix[i_1,i_2] = 2-abs(RT_List_1[Sm_Filter[i_1][0]]-self.TargetList_set.loc[i,'RT'][i_2]*60)/ self.get_param('List_RT_Tor') - abs(RT_List_2[Sm_Filter[i_1][1]]-self.TargetList_set.loc[i,'RT'][i_2]*60)/ self.get_param('List_RT_Tor')
                    Sm_1,Sm_2 = linear_sum_assignment(Score_matrix,True)         
                    ...
                    if len(FilePath)>0:
                        ...
                        for i_1,i_2 in Sm_Assign:
                            fig_1.add_trace(
                                go.Scatter(
                                    x=list(MRM_1.loc[:,'RTList']/60)[0][Peak_Begin_1[Sm_Filter[i_1][0]]:Peak_End_1[Sm_Filter[i_1][0]]+1],
                                    y=list(MRM_1.loc[:,'Int'])[0][Peak_Begin_1[Sm_Filter[i_1][0]]:Peak_End_1[Sm_Filter[i_1][0]]+1],
                                    mode='lines',
                            ...
                            fig_2.add_trace(
                                go.Scatter(
                                    x=list(MRM_2.loc[:,'RTList']/60)[0][Peak_Begin_2[Sm_Filter[i_1][1]]:Peak_End_2[Sm_Filter[i_1][1]]+1],
                                    y=list(MRM_2.loc[:,'Int'])[0][Peak_Begin_2[Sm_Filter[i_1][1]]:Peak_End_2[Sm_Filter[i_1][1]]+1],
                                    mode='lines',
                                    name=self.TargetList.loc[self.TargetList_set.loc[i,'TL_Index'][i_2],'Identity keys'],
                                    line={'width':1},
                                    fill='tozeroy',
            ...
            for i in range(len(self.TargetList)):  
                if np.isnan(self.TargetList.loc[i,'Area_1']) == True and np.isnan(self.TargetList.loc[i,'Edge_left_1']) == False:
                    ...
                    MRM_2 = self.Auto_Peak[(abs(self.Auto_Peak['Pre_MZ']-self.TargetList.loc[i,'2_Q1'])<=0.35)&(abs(self.Auto_Peak['Pro_MZ']-self.TargetList.loc[i,'2_Q3'])<=0.35)]
                    if len(MRM_1)>0 and len(MRM_2)>0:
                        Noise_1 = self.Calculate_Noise(MRM_1.iloc[0,:]['RTList'],MRM_1.iloc[0,:]['Int'])
                        Noise_2 = self.Calculate_Noise(MRM_2.iloc[0,:]['RTList'],MRM_2.iloc[0,:]['Int'])
